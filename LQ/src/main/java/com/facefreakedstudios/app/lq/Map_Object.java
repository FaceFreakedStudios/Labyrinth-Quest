...
abstract class Map_Object
{
        ...
        throws IOException // without extensions
    {
        ...
        map = new_map;
        orig_map = LQCLI.fetchMap("/home/gavin18/Scripts/Java/Labyrinth-Quest"
                    + "/LQ/src/main/resources/Maps/" + new_map + ".map");
        cur_map = LQCLI.fetchMap("/home/gavin18/Scripts/Java/Labyrinth-Quest"
                    + "/LQ/src/main/resources/Maps/" + new_map + ".map");
        map_data = LQCLI.fetchMapData(lucas, "/home/gavin18/Scripts/Java"
    ...
    {
        this.setCurrentBlk(orig_map[this.getPosit()[0]][this.getPosit()[1]]);
        this.setCurrentBlkDat(map_data[this.getPosit()[0]][this.getPosit()[1]]);
    }
    ...
    protected boolean canMove(Lucas lucas, int x, int y)
    {
        ...
        if(x >= 0) // X moves first
        {
            for(int i = 0; i < x + 1; ++i)
            {
                switch(orig_map[this.getPosit()[0] + i][this.getPosit()[1]])
                {
                    case "#":
        ...
        if(y >= 0)
        {
            for(int i = 0; i < y + 1; ++i)
            {
                switch(orig_map[this.getPosit()[0] + x][this.getPosit()[1] + i])
                {
                    case "#":
                        LQOS.outError("Cannot walk on walls");
                        return false;
    ...
    protected boolean canMove(Enemy ene, int x, int y)
    {
        ...
        if(this.positx  + y < 0 // to stay within the array
            || this.positx + y > 60 
            || this.posity + x < 0 
            || this.posity + x > 32)
        { 
